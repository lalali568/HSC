...
class SSHSR_1(nn.Module):#这个是tcn的版本
    def __init__(self,config):
        super(SSHSR_1, self).__init__()
...
class GATModel(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.lin(x)
        x=x.view(batch_size,-1,self.config['input_dim'])
        return x
class SSHSR_2(nn.Module):
    def __init__(self,config):
        super(SSHSR_2,self).__init__()
        self.config = config
        ...
        self.time_expand = nn.Linear(config['base_length'], config['base_length']+config['fore_length'], bias=True)
    def forward(self,x):
        x=x.to(torch.float)
        x=x.permute(0,2,1)
        x = self.sigmoid((self.time_expand(x)))
        x = x.permute(0,2,1)
        out = self.gat1(x)
...
#%%这是最后总体的框架
class SSHSR(nn.Module):
    def __init__(self,config):
        super(SSHSR,self).__init__()
        self.hypersphere_layer = SSHSR_1(config)
        self.decoder_layer = SSHSR_2(config)
