...
class GATModel(nn.Module):
    def __init__(self,config,in_features,out_features):
        ...
        self.config = config
        self.conv = HSC_GAT.GATConv(in_features, out_features, heads=config['num_heads'], dropout=config['dropout'],)
...
class HSC_2(nn.Module):
    def __init__(self,config):
        super(HSC_2,self).__init__()
...
class HSC(nn.Module):
    def __init__(self,config):
        ...
        self.hypersphere_layer = HSC_1(config)
        self.decoder_layer = HSC_2(config)
