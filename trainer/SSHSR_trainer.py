...
def trainer(config,model1, model, train_dataloader, optimizer, device):
    ...
    fore_criterion = nn.MSELoss(reduction='none')
    model1.train()
    loss_hypersphere=[]
    recon_loss = []
    fore_loss = []
    ...
    with tqdm(total=len(train_dataloader) * config['epoch'], unit='batch', leave=True) as pbar:
        for epoch in range(config['epoch']):
            ...
            for ii, x0 in enumerate(train_dataloader):
                ...
                #把输入和target分开
                target=x0[:,-(config['base_length']+config['fore_length']):,:]
                x0=x0[:,:config['base_length'],:]
                optimizer.zero_grad()
                rep,output = model(x0,c)
                loss_model1 = (torch.mean(torch.sum((rep - c) ** 2, dim=1))-config['radius'])**2
                loss_model2 = recon_criterion(output, target)
                ...
                pbar.update(1)
                pbar.set_description(f"loss_model1:{loss_model1},loss_model2:{loss_model2}")
