...
sed -i "s#train_data_path:.*#train_data_path: $new_train_data#" config/TranAD/config.yaml
# 注释和取消注释训练的代码
comment_lines() {
  sed -i "$1s/^/#/" main.py
}
...
run_test_data() {
  ...
  sed -i "s#test_data_path:.*#test_data_path: $test_data_path #" config/TranAD/config.yaml
